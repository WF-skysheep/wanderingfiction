...
function createApp({ db, aiProvider, corsOrigin } = {}) {
  ...
  const cookieSameSite = String(process.env.WF_COOKIE_SAMESITE || (isProd ? 'none' : 'lax')).toLowerCase();
  const cookieSecure = String(process.env.WF_COOKIE_SECURE || (isProd ? 'true' : 'false')).toLowerCase() !== 'false';
  ...
  app.post('/wf/api/auth/logout', requireAuth, (req, res) => {
    ...
    res.clearCookie('wf_sid', { httpOnly: true, sameSite: cookieSameSite, secure: cookieSecure });
    res.json({ ok: true });
  });
